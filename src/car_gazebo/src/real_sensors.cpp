...
void imageCallbackdepth(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
    ...
    imwrite("/home/project/car_garzebo/test_results/depth.png",depthImg);
...
int getpix(string imgstr)
{ 	    
//	src = imread( "/home/lee/projects/midext/color.png" ,CV_LOAD_IMAGE_COLOR ); 	//注意路径得换成本身的
...
int main(int argc, char **argv)
{
  ...
  image_transport::ImageTransport it1(nh1);//使用ImageTransport方法建立图像发布者和订阅者，就像咱们使用NodeHandle方法建立通常ROS发布者和订阅者同样
  image_transport::ImageTransport it2(nh2);
  image_transport::Subscriber sub1 = it1.subscribe("/real_sense/rgb/image_raw", 1, imageCallbackrgb);
  image_transport::Subscriber sub2 = it2.subscribe("/real_sense/depth/image_raw", 1, imageCallbackdepth);
